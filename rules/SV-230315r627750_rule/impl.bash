...
# Severity:               CAT II
#
# Rule Version (STIG-ID): RHEL-08-010675
#
# Rule Title:             RHEL 8 must disable core dump backtraces.
#


config_file="50-accident-stig-disable-coredump-backtraces.conf"
...
# if no pending reboot, check compliance now
	...
	if [ -e "/etc/systemd/coredump.conf.d/$config_file" ]; then
		compliance="$ACCIDENT_CONST_COMPLIANT"
	else
	...
	if [ "$compliance" = "$ACCIDENT_CONST_NONCOMPLIANT" -a "$ACCIDENT_STIG_IMPL_ALLOW_IMPLEMENT" -ne 0 ]; then
		...
		if [ ! -e "/etc/systemd/coredump.conf.d/$config_file" ]; then
			...
			if [ ! -d "/etc/systemd/coredump.conf.d" ]; then
				trace mkdir "/etc/systemd/coredump.conf.d"
			fi

			trace cp -nv "$ACCIDENT_STIG_IMPL_RULE_DIR_RES/$config_file" -T "/etc/systemd/coredump.conf.d/$config_file"
...
compliance --rule-id "SV-230315r627750_rule" --status "$compliance"
